...
class MoveHistory(tk.Frame):
    def __init__(self, parent, board_canvas, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.board_canvas = board_canvas
        self.game = chess.pgn.Game()  # Initialize an empty PGN game
        self.board = chess.Board()  # Initialize a board for FEN conversion
        self.moves = []
        self.current_move_index = -1  # Start before the first move
        self.is_current = True  # Flag to indicate if showing the current board
    ...
    def load_moves(self, pgn_string):
        """Load moves from a PGN string."""
        self.game = chess.pgn.read_game(io.StringIO(pgn_string))
        self.board = self.game.board()  # Start with the initial position
        self.moves = list(self.game.mainline_moves())  # Extract all moves
